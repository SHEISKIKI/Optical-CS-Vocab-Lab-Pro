...
<head>
  ...
  <title>Optical CS Vocab Trainer — Pro</title>
  <style>
    :root{
      --bg:#0f172a;         /* slate-900 */
  ...
  <header>
    <div class="wrap row">
      <div class="brand">
        ...
        <div>
          <h1>Optical CS Vocab Lab — Pro</h1>
          <div class="muted small">Bilingual • TTS • Voice input • SRS review</div>
        </div>
      </div>
...
const DATA = {
  categories: {
    "Ordering": [
      { key:"prescription", head:"prescription", en:"a written order from an eye doctor specifying lens power", zh:"处方；配镜参数（由眼科医生出具）", exEN:"Please upload your prescription before we process your order.", exZH:"在处理订单前请上传您的处方。", phrases:["Could you upload your prescription?","The prescription must be valid within 12 months."] },
      { key:"frame", head:"frame", en:"the structure holding the lenses of glasses", zh:"镜架；承托镜片的结构", exEN:"This frame is lightweight and suitable for daily wear.", exZH:"这副镜架很轻，适合日常佩戴。", phrases:["This frame fits medium-narrow faces.","The frame material is acetate."] },
      { key:"customization", head:"customization", en:"tailoring a product to specific needs", zh:"定制；个性化配置", exEN:"We offer customization for lens thickness and coatings.", exZH:"我们提供镜片厚度与镀膜的定制。", phrases:["Would you like blue‑light filtering?","You can customize temple length."] },
    ],
    "Shipping & Logistics": [
      { key:"processing-time", head:"processing time", en:"time required before an order is shipped", zh:"处理时间；生产周期", exEN:"The processing time for prescription lenses is 5–7 business days.", exZH:"配镜的处理时间为 5–7 个工作日。", phrases:["Standard processing takes 5–7 business days.","We'll notify you once shipped."] },
      { key:"fulfillment", head:"fulfillment", en:"the process of preparing and shipping orders", zh:"订单履行；发货处理", exEN:"Your order is now in fulfillment and will be shipped shortly.", exZH:"您的订单正在处理，即将发货。", phrases:["Your order is awaiting fulfillment.","Picked and packed."] },
      { key:"tracking", head:"tracking number", en:"a unique code to trace the shipment status", zh:"物流跟踪号", exEN:"We'll email the tracking number once the parcel leaves the warehouse.", exZH:"包裹一出库我们会邮件发送跟踪号。", phrases:["Here's your tracking number.","The carrier has accepted the parcel."] },
    ],
    "Returns & Warranty": [
      { key:"warranty", head:"warranty", en:"a guarantee to repair or replace defective products", zh:"保修；质保", exEN:"This frame comes with a one‑year warranty.", exZH:"这副镜架享有一年的保修期。", phrases:["Manufacturing defects are covered.","Warranty does not cover accidental damage."] },
      { key:"return-policy", head:"return policy", en:"the rules for returning or exchanging products", zh:"退货/换货政策", exEN:"Our return policy allows exchanges within 30 days.", exZH:"我们的退货政策允许在 30 天内更换。", phrases:["Returns must be in original condition.","We can provide a return label."] },
      { key:"resolution", head:"resolution", en:"the act of solving a problem or dispute", zh:"解决方案；问题处理", exEN:"We will provide a prompt resolution to your concern.", exZH:"我们会尽快为您解决问题。", phrases:["Let me propose a resolution.","We can offer a replacement or refund."] },
    ],
    "Optics & Measurement": [
      { key:"pd", head:"pupillary distance (PD)", en:"the distance between the centers of the pupils", zh:"瞳距（PD），双眼瞳孔中心间距", exEN:"We need your PD to ensure proper lens alignment.", exZH:"我们需要您的瞳距以确保镜片准确对齐。", phrases:["You can measure PD using our guide.","Ask your optometrist for your PD."] },
      { key:"progressives", head:"progressive lenses", en:"lenses that correct multiple focal distances seamlessly", zh:"渐进多焦镜片", exEN:"Progressive lenses provide a seamless transition between near and far.", exZH:"渐进镜片能在远近视力间无缝过渡。", phrases:["Short adaptation period.","No visible line on the lens."] },
      { key:"coating", head:"lens coating", en:"a thin layer added to lenses to improve performance", zh:"镜片镀膜/涂层（如防反射、抗紫外线）", exEN:"Would you like to add a blue‑light coating?", exZH:"您想加防蓝光镀膜吗？", phrases:["Anti‑reflective reduces glare.","UV protection up to 400 nm."] },
    ],
  }
};
...
function learn(){
  ...
  top.appendChild(el('h2',null,'Vocabulary — Learn by category'));
  ...
  const hint = el('div','muted small','Tip: Use 🔊 to hear English; click ✓ when you master a word.');
...
function renderWordCard(w){
  ...
  const say = el('button','btn', '🔊 Speak'); say.onclick=()=> speak(`${w.head}. ${w.en}. Example: ${w.exEN}`);
  const mark = el('button','btn ok', '✓ Mastered'); mark.onclick=()=> { profile.learned[w.key]=true; // update SRS as "good"
  ...
  ex.appendChild(el('div','muted small','例句（中文）'));
...
function quiz(){
  ...
  box.appendChild(el('h2',null,'Quiz — 10 questions'));
  const qEl = el('div','');
  ...
  function pick(ok){
    res.textContent = ok? '✔ Correct':'✖ Wrong'; res.style.color = ok? 'var(--ok)':'var(--err)';
...
function roleplay(){
  clear(view);
  const box = el('div','card');
  box.appendChild(el('h2',null,'Role‑play — Customer Service'));

  const cust = el('div','mono');
  const tips = el('div','muted small');
  const area = el('textarea'); area.placeholder = 'Type your reply as the agent…';

  const actions = el('div','row-actions');
  const speakBtn = btn('🔊 Speak customer line', ()=> speak(cust.textContent.replace('Customer: ','')) );
  const micBtn = btn('🎤 Voice input', startVoice);
  ...
  function startVoice(){
    ...
    micBtn.textContent='🎙️ Listening… (speak in English)'; micBtn.disabled=true; area.value='';
    recognizer.onresult = (e)=>{ const txt = Array.from(e.results).map(r=>r[0].transcript).join(' '); area.value = txt; };
    recognizer.onend = ()=>{ micBtn.textContent='🎤 Voice input'; micBtn.disabled=false; };
    recognizer.onerror = ()=>{ micBtn.textContent='🎤 Voice input'; micBtn.disabled=false; toast('Mic error') };
  ...
  function nextTurn(){
    ...
    fb.textContent = (ok? '✔ Good: you used relevant service vocabulary.' : '✖ Try to include: '+ s.kws.join(', '));
...
function review(){
  clear(view);
  const box = el('div','card');
  box.appendChild(el('h2',null,'Review — Spaced Repetition'));
  const due = reviewDue();
  ...
  function render(){
    ...
    const actions = el('div','row-actions');
    const say = btn('🔊 Speak', ()=> speak(`${w.head}. ${w.en}. Example: ${w.exEN}`));
...
function settings(){
  ...
  const sel = el('select'); [{v:'en',t:'English'},{v:'zh',t:'中文 Chinese'}].forEach(o=>{const op=document.createElement('option'); op.value=o.v; op.textContent=o.t; sel.appendChild(op)});
  ...
  const tts = el('div','row'); tts.appendChild(el('div','muted small','TTS uses your browser’s speechSynthesis. Voice depends on OS.'));
